...
public class WorkRoutineManager extends BaseManager{
    ...
    public void performRoutineSaveOperations() {
        WorkRoutineDAO workRoutineDAO = new WorkRoutineDAO();
        WorkGroupDAO workGroupDAO = new WorkGroupDAO();
        WorkCardDAO workCardDAO = new WorkCardDAO();
        int routineIdAutoGeneratedByDB = workRoutineDAO.saveWorkRoutine(workRoutineModel);
        ...
        for (WorkGroup wg: workGroups) {
            workGroupIdAutoGeneratedByDB = workGroupDAO.saveWorkGroup(wg, routineIdAutoGeneratedByDB);
            workCards = wg.getWorkCardList();
            for (WorkCard wc: workCards) {
                workCardDAO.saveWorkCard(wc, workGroupIdAutoGeneratedByDB);
            }
        }
    }
}
